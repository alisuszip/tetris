...
class AppModel {
    ...
    private fun blockAdditionPossible(): Boolean {
        if (!moveValid(currentBlock?.position as Point, currentBlock?.frameNumber) as Boolean) {
            return false
        }
        return true
    }
    ...
    companion object {
        private fun assessField(appModel: AppModel) {
            for (i in 0 until appModel.field.size) {
                var emptyCells = 0
                for (j in 0 until appModel.field[i].size) {
                    val status = appModel.getCellStatus(i, j)
                    val isEmpty = CellConstants.EMPTY.value == status
                    if (isEmpty)
                        emptyCells++
                }
                if (emptyCells == 0)
                    appModel.shiftRows(i)
            }
        }
    }
}
